...
import { userDepartmentRoutes } from './user.routes.js';

describe('userDepartmentRoutes', () => {
  ...
  beforeEach(async () => {
    ...
    const authenticatedRoute: FastifyPluginAsync = async (fastify) => {
      ...
      await fastify.register(userDepartmentRoutes);
    };
  ...
  test('fails loudly when mounted without the authentication hook', async () => {
    await app.close();
    app = await createApiRouteApp(userDepartmentRoutes, {
      user: null,
      db: {
        userDepartment: {
          findMany,
